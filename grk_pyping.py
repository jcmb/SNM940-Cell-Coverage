...
class Ping(object):
	...
	def print_exit(self):
		...
		if self.send_count == 0 : # Can get 0 with general failures
			lost_rate = 100.0
		else:
	...
	def do(self):
		...
		self.send_count += 1
# If the OS returns host is down, or if the network is down we still failed to ping.

		if send_time == None:  # If the send fails we now count it as a send. 
			return
	...
	def send_one_ping(self, current_socket):
		...
		try:
			...
			if self.quiet_output: #GRK: Handle General Failures when showing output correctly
